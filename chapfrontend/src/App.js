  ...
  return (
    <Router history={history}>
      <div className="chat-frame">
        <label className="title">ChatApp<span><WhatsAppIcon></WhatsAppIcon></span></label>  
        <Route exact path='/'> 
          <img src={usc} alt="USC" className="usc"></img>
          <User userName={userName} login={login}></User>
        </Route> 
        <Route path='/room'>
          <Room loginName={nameId}  checkLogin={isLoggedIn} roomIn={roomIn} participateInRoom={participateInRoom}></Room>
        </Route>
        <Route path='/message'>
          <Message name={nameId} room={roomJoin}  checkLogin={isLoggedIn}></Message>
        </Route>
        {isLoggedIn &&　!participate ?　 
        <Redirect to={URL}></Redirect>: null} 
        {isLoggedIn &&　participate ? <Redirect to={messageURL}></Redirect> : null}   
      </div>  
    </Router>
